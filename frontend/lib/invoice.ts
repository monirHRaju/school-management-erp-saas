...
export interface MultiInvoiceData {
  receiptNo: string;
  paymentDate: string | Date;
  collectedBy?: string;
  items: MultiInvoiceItem[];
  student: InvoiceData['student'];
  school: InvoiceData['school'];
}

function multiCopyHtml(label: string, d: MultiInvoiceData): string {
  const dateStr = new Date(d.paymentDate).toLocaleString('en-GB', {
  ...
  const logoHtml = d.school.logoUrl
    ? `<img src="${d.school.logoUrl}" alt="logo" class="logo" />`
    : `<div class="logo-placeholder">${(d.school.name || 'S')[0].toUpperCase()}</div>`;
  const totalPaid = d.items.reduce((s, i) => s + i.payment.amount, 0);
  const totalDiscount = d.items.reduce((s, i) => s + (i.payment.discount || 0), 0);

  const itemRows = d.items.map((item, idx) => `
    <tr>
      <td class="num">${idx + 1}</td>
      <td>
        <b>${formatCategory(item.fee.category)}</b>${item.fee.month ? ' — ' + item.fee.month : ''}
        ${item.fee.description ? '<br/><span class="muted">' + item.fee.description + '</span>' : ''}
        ${item.payment.note ? '<br/><span class="muted">Note: ' + item.payment.note + '</span>' : ''}
      </td>
      <td class="num">${tk(item.payment.amount)}</td>
    </tr>
  `).join('');
  ...
  const discountRow = totalDiscount > 0 ? `
    <tr>
      ...
      <td class="num">- ${tk(totalDiscount)}</td>
    </tr>` : '';
  ...
  return `
    <section class="copy">
      ...
      <table class="items">
        <thead>
          <tr><th class="num">#</th><th>Description</th><th class="num">Amount (BDT)</th></tr>
        </thead>
        <tbody>${itemRows}${discountRow}</tbody>
        <tfoot>
          <tr>
            <td colspan="2" class="total-lbl">Total Paid</td>
            <td class="num total-val">${tk(totalPaid)}</td>
          </tr>
        </tfoot>
      </table>
      <div class="sign-row">
...
export function buildMultiInvoiceHTML(data: MultiInvoiceData): string {
  return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8" />
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .sign-line { border-bottom: 1px solid #111; height: 24px; }
  .sign-label { font-size: 10px; text-align: center; margin-top: 4px; color: #333; }
  @media print { .no-print { display: none !important; } .copy { page-break-inside: avoid; } }
  .toolbar { position: fixed; top: 8px; right: 8px; display: flex; gap: 6px; z-index: 10; }
  .toolbar button { padding: 6px 14px; border: 1px solid #888; background: #fff; cursor: pointer; border-radius: 4px; font-size: 12px; }
  .toolbar button.primary { background: #111; color: #fff; border-color: #111; }
</style>
...
<body>
  ...
  <div class="sheet">
    ${multiCopyHtml('Student Copy', data)}
    ${multiCopyHtml('Office Copy', data)}
  </div>
</body>
</html>`;
}

export function openMultiInvoiceWindow(data: MultiInvoiceData): void {
  const html = buildMultiInvoiceHTML(data);
